 ...
 */
class SmallStatsWidget : BaseAppWidgetProvider() {
    override fun onUpdate(
        ...
        for (appWidgetId in appWidgetIds) {
            updateSmallStatsAppWidget(context, appWidgetManager, appWidgetId)
        }
    }
    ...
    fun updateSmallStatsAppWidget(
            context: Context,
            appWidgetManager: AppWidgetManager,
            appWidgetId: Int
    ) {
        CoroutineScope(Dispatchers.Main.immediate).launch {
            val views = RemoteViews(context.packageName, R.layout.small_stats_widget)
